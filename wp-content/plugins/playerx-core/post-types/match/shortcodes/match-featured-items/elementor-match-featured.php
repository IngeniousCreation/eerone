...
class ElementorEdgtfMatchFeatured extends \Elementor\Widget_Base {

	public function get_name() {
		return 'edgtf_match_featured'; 
	}

	public function get_title() {
		return esc_html__( 'Match Featured Items', 'playerx-core' );
	}

	public function get_icon() {
		return 'playerx-elementor-custom-icon playerx-elementor-match-featured-items';
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			[
				'label'     => esc_html__( 'Image Dimensions', 'playerx-core' ),
				'type'      => \Elementor\Controls_Manager::TEXT,
				'description' => esc_html__( 'Enter custom image dimensions. Enter image size in pixels: 200x100 (Width x Height)', 'playerx-core' )
			]
		);
	...
	public function render() {
		...
		if ( $queryResults->have_posts() ) {
			while ( $queryResults->have_posts() ) {
				...
				$html .= playerx_core_get_cpt_shortcode_module_template_part( 'match', 'match-featured-items', 'match', '', $new_params );
			}
		...
		$html .= '</div>'; //close edgtf-match-featured-holder
	...
	public function getMatchHolderClasses( $params ) {
		...
		$classes[] = 'edgtf-match-featured-holder';
...
\Elementor\Plugin::instance()->widgets_manager->register_widget_type( new ElementorEdgtfMatchFeatured() );

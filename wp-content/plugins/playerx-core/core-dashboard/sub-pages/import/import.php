...
class PlayerxCoreImport {
	...
	public function set_import_url() {
		$params = PlayerxCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'edgtf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (playerx_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'playerx-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!PlayerxCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									playerx_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'playerx-core'));
									exit;
								}
							}
			...
			playerx_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'playerx-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $playerx_widget_id => $playerx_widget_data) {
				update_option('widget_' . $playerx_widget_id, $playerx_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'playerx-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$playerx_sidebars = get_option( "sidebars_widgets" );
		unset( $playerx_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$playerx_sidebars = array_merge( (array) $playerx_sidebars, (array) $data['sidebars'] );
			unset( $playerx_sidebars['wp_inactive_widgets'] );
			$playerx_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $playerx_sidebars );
			$playerx_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $playerx_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(PLAYERX_CORE_OPTIONS_NAME);

		if($options){
			if($current_options != $options) {
				$result = update_option(PLAYERX_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'playerx-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'playerx-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'playerx-core'));
			}
		}
        if(playerx_edge_is_plugin_installed('elementor')){
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_playerx_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'playerx-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'playerx-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'playerx-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'playerx-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'playerx-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( PLAYERX_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(playerx_core_is_woocommerce_installed()) {
		...
		$playerx_import = new WP_Import();
		set_time_limit( 0 );

		$playerx_import->fetch_attachments = $attachments;
		$returned_value                  = $playerx_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'playerx-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'playerx-core' ) . ' ' . $xml );
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'playerx-core'));
					exit;
				}
			}
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'playerx-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'playerx-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( PLAYERX_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( PLAYERX_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return PLAYERX_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'edgtf_cd_import_nonce' ) ) {
				...
				$html = playerx_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					playerx_core_ajax_status( 'success', '', $html);
				} else {
					playerx_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'playerx-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = PlayerxCoreSystemInfoPage::get_instance()->get_system_info();
...
PlayerxCoreImport::get_instance();
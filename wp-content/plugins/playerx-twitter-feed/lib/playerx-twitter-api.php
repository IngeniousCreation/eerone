...
include_once 'playerx-twitter-helper.php';

/**
 * Class PlayerxTwitterApi
 */
class PlayerxTwitterApi {
    /**
    ...
    const REQUEST_TOKEN_FIELD = 'playerx_twitter_request_token';
    const REQUEST_TOKEN_SECRET_FIELD = 'playerx_twitter_request_token_secret';
    const ACCESS_TOKEN_FIELD = 'playerx_twitter_access_token';
    const ACCESS_TOKEN_SECRET_FIELD = 'playerx_twitter_access_token_secret';
    const AUTHORIZE_TOKEN_FIELD = 'playerx_twitter_authorize_token';
    const AUTHORIZE_VERIFIER_FIELD = 'playerx_twitter_authorize_verifier';
    const USER_ID_FIELD = 'playerx_twitter_user_id';
    const USER_SCREEN_NAME_FIELD = 'playerx_twitter_screen_name';
    ...
    public function __construct() {
	    ...
	    $this->redirectURI = defined( 'EDGE_PROFILE_SLUG' ) ?  'http://demo.' . EDGE_PROFILE_SLUG . '-themes.com/twitter-app/twitter-redirect.php' : '';
        ...
        $this->helper = new PlayerxTwitterHelper();
    }

    /**
     * @return PlayerxTwitterApi
     */
     ...
     *
     * @see PlayerxTwitterApi::generateSignatureBase()
     */
    ...
    public function obtainRequestToken() {
        ...
        if(is_wp_error($response)) {
            ...
            $responseObj->message = esc_html__('Internal WP error', 'playerx-twitter-feed');
        } else {
            ...
            if(!empty($responseBody)) {
                ...
                if((is_array($responseParsed) && count($responseParsed)) &&
                    ...
                    if(!empty($responseObj->redirectUrl)) {
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Redirect URL couldn\t not be generated', 'playerx-twitter-feed');
                    } else {
                    ...
                    $responseObj->status = false;
                    $responseObj->message = esc_html__('Couldn\'t connect with Twitter API', 'playerx-twitter-feed');
                }
            }
        }
        ...
        if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
            ...
            if(is_wp_error($response)) {
                ...
                if(is_array($responseParsed) && count($responseParsed)
                    ...
                    $responseObj->status = true;
                    $responseObj->message = esc_html__('Access token obtained', 'playerx-twitter-feed');
                }
            }
        } else {
            $responseObj->status = false;
            $responseObj->message = esc_html__('Authorize token and it\'s secret were not obtainer', 'playerx-twitter-feed');
        }
    ...
    public function fetchTweets($userId = '', $count = '', $transient = array()) {
        ...
        if(!$this->transientExists($transient)) {
            if($userId && $accessToken) {
                ...
                if(is_wp_error($response)) {
                    ...
                    if(isset($response['response']['code']) && $response['response']['code'] == 200) {
                        ...
                        $responseObj->status = false;
                        $responseObj->message = esc_html__('Couldn\'t connect with Twitter', 'playerx-twitter-feed');
                    }
                }
            } else {
                $responseObj->status = false;
                $responseObj->message = esc_html__('It seams like you haven\t connected with your Twitter account', 'playerx-twitter-feed');
            }
        } else {
            ...
            if(!empty($transientContent)) {
                ...
                $responseObj->status = false;
                $responseObj->message = esc_html__('Couldn\'t retreive content from database', 'playerx-twitter-feed');
            }
        }
